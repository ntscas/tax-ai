...
// 내부적으로 기본 사용할 GCP 버킷 파일 URI 설정 (UI에는 표시되지 않음)
// 주의: 실제 버킷에 존재하는 파일명(예: tax_data.pdf, data.csv 등)으로 반드시 수정해야 합니다.
const DEFAULT_BUCKET_URI = 'gs://ntis-tax-kr/tax_data.pdf';
...
[중요 내부 프로세스]: 판례, 유권해석, 예규 등을 인용할 경우에는 반드시 제공된 'ntis-tax-kr'의 데이터와 상호 검증(Cross-verification)을 거친 후 그 결과를 반영하여 답변을 작성하세요. 또한, 인용하는 모든 판례, 유권해석, 예규 등에는 반드시 해당 문서의 고유 번호(예: 사건번호, 문서번호, 예규번호 등)를 명시해야 합니다.
...
[중요 내부 프로세스]: 판례, 유권해석, 예규 등을 인용할 경우에는 반드시 제공된 'ntis-tax-kr'의 데이터와 상호 검증(Cross-verification)을 거친 후 그 결과를 반영하여 작성하세요. 또한, 인용하는 모든 판례, 유권해석, 예규 등에는 반드시 해당 문서의 고유 번호(예: 사건번호, 문서번호, 예규번호 등)를 명시해야 합니다.
...
전문적이고 논리적인 법률 문장으로 작성하며, 마크다운 형식을 사용하여 가독성을 높이세요.`;
  ...
  try {
    // Construct parts array with the default bucket data and user query
    const parts: any[] = [];

    // 기본 GCP 버킷 데이터 연동
    if (DEFAULT_BUCKET_URI) {
      let mimeType = 'text/plain';
      if (DEFAULT_BUCKET_URI.toLowerCase().endsWith('.pdf')) mimeType = 'application/pdf';
      else if (DEFAULT_BUCKET_URI.toLowerCase().endsWith('.csv')) mimeType = 'text/csv';
      else if (DEFAULT_BUCKET_URI.toLowerCase().endsWith('.json')) mimeType = 'application/json';

      parts.push({
        fileData: {
          fileUri: DEFAULT_BUCKET_URI,
          mimeType: mimeType
        }
      });
    }
    ...
    // 버킷 파일 접근 오류에 대한 명확한 안내
    if (errorStr.includes('403') || errorStr.includes('404') || errorStr.includes('not found')) {
      throw new Error(`[GCP 버킷 연동 오류]\n기본 설정된 파일(${DEFAULT_BUCKET_URI})을 찾을 수 없거나 접근 권한이 없습니다.\n\n해결방법: 소스 코드(services/gemini.ts)의 DEFAULT_BUCKET_URI 값을 실제 존재하는 파일명으로 수정해주세요.`);
    }
